...
class VideoViewController: UICollectionViewController {
    ...
    private func setupData() {
        Task {[weak self] in
            let result = await self?.viewModel.fetchMoreData()
            Task {@MainActor in
                if result?.isEmpty ?? false {
                    self?.collectionView.reloadData()
                } else {
                    self?.showMessageBaseOn(type: .error, message: result ?? "Unknown error")
                }
    ...
    private func loadMoreData() {
        LoadingView.shared.startLoadingVia(parentView: view)
        Task {[weak self] in
            self?.viewModel.updateLoadingData(true)
            let result = await self?.viewModel.fetchMoreData()
            Task { @MainActor in
                if result?.isEmpty ?? false {
                    self?.collectionView.reloadData()
                    self?.viewModel.updateLoadingData(false)
                } else {
                    self?.showMessageBaseOn(type: .error, message: result ?? "Unknown error")
                }
                LoadingView.shared.stopLoading()
